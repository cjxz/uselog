...
public class BufferUtils {
    ...
    private static final ThreadLocal<char[]> CHAR_ARRAY_CACHE = new ThreadLocal<>();
    ...
    public static BufferResult writeStringToBuffer(String str, byte[] destBuffer, int startPos) {
        int strLength = str.length();
        int expectLength = startPos + (strLength << 1);
        byte[] result = ensureBufferLength(destBuffer, expectLength);

        for (int i = 0; i < strLength; i++) {
            char c = str.charAt(i);
            result[startPos++] = (byte) (c >> 8);
            result[startPos++] = (byte) (c & 0x00FF);
        }
        return new BufferResult(startPos, result);
    }
    ...
    public static String readString(byte[] bytes) {
        ...
        char[] buff = CHAR_ARRAY_CACHE.get();
        if (isNull(buff) || buff.length < length) {
            buff = new char[length];
            CHAR_ARRAY_CACHE.set(buff);
        }
        return readString(bytes, buff);
    }
